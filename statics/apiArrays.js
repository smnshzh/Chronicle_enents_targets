...
productxmlhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
        ...
        for (let i = 0; i < product.length; i++) {
            productDict[product[i].id_code+'-'+product[i].name] = product[i];
            productDictPrice[product[i].id_code+'-'+product[i].name] = product[i].price;
            productDictBox[product[i].id_code+'-'+product[i].name] = product[i].box;
            productsList.push(product[i].id_code+'-'+product[i].name);
        }
    }
};
